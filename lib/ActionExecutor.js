(function (root, factory) {
    if (typeof exports === 'object') {
        module.exports = factory(require('./ActionNotReadyError'), require('./ActionCancelledError'));
    } else if (typeof define === 'function' && define.amd) {
        define(['./ActionNotReadyError', './ActionCancelledError'], factory);
    } else {
        root.com = root.com || {};
        root.com.one = root.com.one || {};
        root.com.one.ActionExecutor = factory(root.com.one.ActionNotReadyError, root.com.one.ActionCancelledError);
    }
}(this, function (ActionNotReadyError, ActionCancelledError) {
    function ActionExecutor(options) {
        var that = this;

        if ('onEmptyQueue' in options) {
        ...
        if ('shouldRetryOnError' in options) {
            that.shouldRetryOnError = options.shouldRetryOnError;
        }

        if ('interceptor' in options) {
